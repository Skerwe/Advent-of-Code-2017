...
public class Day12 {
    ...
    private static String input = "0 <-> 659, 737\n" +
            "1 <-> 1, 1433\n" +
            "2 <-> 982, 1869\n" +
            "3 <-> 306, 380, 1462, 1827\n" +
            "4 <-> 1076\n" +
    ...
    public static void main(String[] args) {
        System.out.println("\nPrograms in the group that contains program ID 0 ="+new Day12().partOne(input, 0));
        System.out.println("\nAnswer="+new Day12().partTwo(input));
    }

    private int partOne(String input, int communicationId) {
        HashMap<Integer, Program> village = new HashMap<>();
        List<Integer> pipeline = new ArrayList<>();

        for (String villager : input.split("\n")) {
            populateVillage(villager, village);
        }

        addToPipeline(village.get(communicationId), pipeline);
    ...
    private int partTwo(String input) {
        ...
        for (Program program : village.values()) {
            if (!pipeline.contains(program.getId())) {
                List<Integer> group = new ArrayList<>();

                addToPipeline(program, group);

                pipeline.addAll(group);
                totalGroups++;
            }
        }
    ...
    private void addToPipeline(Program program, List<Integer> pipeline) {
        if (!pipeline.contains(program.getId())) {
            pipeline.add(program.getId());
        }

        for (Program child : program.getChildren()) {
            if (!pipeline.contains(child.getId())) {
                addToPipeline(child, pipeline);
            }
        }
    }

    private void populateVillage(String villager, HashMap<Integer, Program> village) {
        String[] data = villager.split(" ");
        int id = Integer.parseInt(data[0]);

        Program program = getProgram(id, village);

        if (data.length >= 3) {
            Program child;
            for (int i = 2; i < data.length; i++) {
                child = getProgram(Integer.parseInt(data[i].replaceAll(",", "")), village);
                program.addChild(child);
            }
        }
    }

    private Program getProgram(int id, HashMap<Integer, Program> village) {
        Program program;
        if (village.containsKey(id)) {
            program = village.get(id);
        } else {
            program = new Program(id);
            village.put(id, program);
        }
        return program;
    }
    ...
    class Program {
        ...
        public boolean addChild(Program program) {
            return this.children.add(program);
        }
    }
}
