    ...
    private String process(String input) {
        ...
        for (Structure tower : towerOfPrograms.values()) {
            if (tower.getLinkNames().size() > 0) {
                for (String linkNmae : tower.getLinkNames()) {
                    Structure child = towerOfPrograms.get(linkNmae);
                    child.setParent(tower);
                    tower.addChildLink(linkNmae, child);
                }
            }

        }

        String rootTower = "NOT_FOUND";
        for (Structure tower : towerOfPrograms.values()) {
//            System.out.println(tower);
            if (tower.getParent() == null) {
                rootTower = tower.getName();
                break;
            }
        }
        ...
        Structure rootStructure = towerOfPrograms.get(rootTower);
        for (Structure child : rootStructure.getLinks().values()) {
            int stackWeight = calcWeight(child);
            System.out.println("Stack ["+child.getName()+"]="+stackWeight);
            if (stableValue == 0) {
...
//                System.out.println(child);
    ...
    private int calcWeight(Structure structure) {
        int totalWeight = structure.getWeight();
